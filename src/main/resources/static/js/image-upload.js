	...
	if (files.length > 0) {
		...
		$.ajax({
			...
			success: function(data) {
				alert(data);
				window.location.href="/ilhajob/dashboard-company-profile"
			},
			error: function() {
				alert("이미지 업로드 error");
			}
...
$('#user-profile-upload-btn').click(function() {
	...
	if (files.length > 0) {
		...
		$.ajax({
			...
			success: function(data) {
				alert(data);
				window.location.href="/ilhajob/candidate-dashboard-profile"
			},
			error: function() {
				alert("이미지 업로드 error");
			}
		...
		success: function(data) {
			...
			if (files.length > 0) {
				...
				$.ajax({
					...
					success: function(data) {
						alert(data);
						location.href="/ilhajob/blog-single?id="+blogId;
					},
					error: function() {
						alert("이미지 업로드 error");
					}
